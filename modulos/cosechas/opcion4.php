...
<section class="content-header">
  <h1>
    <i class="fa fa-leaf"></i>&nbsp;Venta de elote y zacate antes de tiempo
  </h1>
  <ol class="breadcrumb">
    ...
    <i class="fa fa-angle-right" style="color: #0C0303"></i>
    &nbsp;&nbsp;
    <li class="active"><i class="fa fa-leaf"></i>&nbsp;Venta de elote y zacate antes de tiempo</li>
  </ol>
</section>
<!-- Main content -->
<section class="content">
  <div class="row">
      <div class="box box-primary">
        <!-- /.box-header -->
        <div class="box-body">
        <form action="" role="form" name="frmopcion4" id="frmopcion4" enctype="multipart/form-data" autocomplete="off" onsubmit="return false">
          <input type="hidden" name="enc_id" id="enc_id" readonly>
          <fieldset>
            <div class="form-group col-md-4">
                <label>Costo total de la siembra: </label>
                <input type="text" class="form-control" name="elote_zacate" id="elote_zacate" readonly placeholder="0.00">
            </div>
            <div class="form-group col-md-4">
                <label>Venta de la siembra: </label>
                <input type="text" class="form-control" name="venta_siembra" id="venta_siembra" placeholder="0.00" data-validation="required" data-validation-error-msg="Complete este campo">
            </div>
            <div class="form-group col-md-4">
                <label>Perdida: </label>
                <input type="text" class="form-control" name="perdida" id="perdida" readonly placeholder="0.00">
            </div>
          </fieldset>
...
//Funcion que calcula la perdida de la cosecha antes del tiempo
$("[name=venta_siembra]").on('change', function(){
  var ctotal = $("[name=elote_zacate]").val();
  var vsiembra = $(this).val();
  $.ajax({
    type: 'POST',
    dataType: 'json',
    url: 'procesos/cosecha/perdida_opcion4.php',
    data: {'ctotal':ctotal, 'vsiembra':vsiembra},
  }).done(function (response){
    document.getElementById('perdida').value = response.total;
  });
});
...
$('#guardar').click(function () {
    $.validate({
        onSuccess: function(form){
            var formulario = document.getElementById("frmopcion4");
            var formData = new FormData(formulario);
            $.ajax({
                url: "procesos/cosecha/guardar_opcion4.php",
                type: "POST",
                dataType: "json",
                data: formData,
                cache: false,
